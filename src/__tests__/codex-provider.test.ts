...
function makeStreamState() {
  return {
    agentTextById: new Map<string, string>(),
...
describe('CodexProvider', () => {
  ...
  it('maps agent_message item to text SSE event', async () => {
    ...
    (provider as any).handleItemEvent(mockController, {
      type: 'agent_message',
      id: 'msg-1',
      text: 'Hello from Codex!',
    }, 'item.completed', makeStreamState());
  ...
  it('streams only the delta for updated agent_message items', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
  ...
  it('emits tool_use at start and avoids duplicating it at completion', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
  ...
  it('collects changed files for outbound delivery', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());

    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cti-codex-out-'));
    const filePath = path.join(tmpDir, 'artifact.txt');
    fs.writeFileSync(filePath, 'artifact-content', 'utf-8');

    const state = makeStreamState();
    state.workingDirectory = tmpDir;
    ...
    const attachment = state.outputFilesByPath.get(filePath);
    assert.ok(attachment);
    assert.equal(attachment.name, 'artifact.txt');
    assert.equal(Buffer.from(attachment.data, 'base64').toString('utf-8'), 'artifact-content');
  });
  ...
  it('does not pass model by default and still attempts resume even if session model is Claude-like', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
    ...
    assert.equal(resumeCalls, 1, 'Should still attempt resume for persisted Codex thread IDs');
    assert.equal(startCalls, 0, 'Should not start a fresh thread when resume succeeds');
    assert.equal(capturedStartOptions, undefined, 'startThread should not be called when resume succeeds');
  });
...
describe('CodexProvider image input', () => {
  ...
  it('passes abort signal into runStreamed so /stop can cancel Codex turns', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
    ...
    const mockThread = {
      runStreamed: async (_input: unknown, turnOptions?: { signal?: AbortSignal }) => {
        receivedSignal = turnOptions?.signal;
        return {
          events: (async function* () {})(),
        };
      },
