...
export interface ConversationResult {
  ...
  hasError: boolean;
  errorMessage: string;
  /** True when the task was stopped by user abort (vs a real error). */
  isAbort: boolean;
  /** Preserve the existing provider session reference after an abort. */
  preserveSdkSession: boolean;
...
export async function processMessage(
  ...
  try {
    ...
    // Also enrich FileAttachment objects with filePath so the LLM provider can
    // tell the model exactly where each file was saved.
    let savedContent = text;
    let enrichedFiles: FileAttachment[] | undefined = files;
    ...
    return await consumeStream(stream, sessionId, abortController.signal, onPermissionRequest, onPartialText, onToolEvent);
  } finally {
...
async function consumeStream(
  ...
  let outputFiles: FileAttachment[] = [];
  ...
  try {
        ...
        switch (event.type) {
          ...
          case 'tool_use': {
            ...
            try {
              ...
              if (onToolEvent) {
                const detail = summarizeToolInput(toolData.name, toolData.input);
                try { onToolEvent(toolData.id, toolData.name, 'running', detail); } catch { /* non-critical */ }
              }
            } catch { /* skip */ }
            break;
          }
              ...
              if (onToolEvent) {
                try {
                  onToolEvent(
                    ...
                    summarizeToolResult(resultData.content),
                  );
                } catch { /* non-critical */ }
              }
            } catch { /* skip */ }
          ...
          case 'result': {
            try {
              ...
              if (Array.isArray(resultData.output_files)) {
                outputFiles = resultData.output_files;
                if (outputFiles.length > 0) {
                  console.log(`[conversation-engine] LLM result contains ${outputFiles.length} output file(s): ${outputFiles.map((f: { name?: string }) => f.name).join(', ')}`);
                }
              }
    ...
    const preserveSdkSession = isAbort && abortSignal?.reason === 'preserve-session';

    if (isAbort) {
      // User-initiated stop: always close the turn with a clean synthetic message.
      //
      // We intentionally skip saving partial contentBlocks here because:
      // 1. Mid-task aborts accumulate tool_use/tool_result blocks that may be
      //    incomplete (e.g. tool_use with no matching tool_result).
      // 2. When those JSON-encoded blocks are later loaded as conversationHistory,
      //    the malformed structure causes LLMs/SDKs to reject the entire history,
      //    wiping all prior context — even from turns that completed normally.
      //
      // Saving a clean closing message ensures the history always alternates
      // user/assistant and the next turn can resume with full context.
      store.addMessage(sessionId, 'assistant', '[Task stopped by user]');
    } else {
      // Real error (not user-initiated): best-effort save of accumulated content
      // so the assistant turn is at least partially recorded for debugging.
      if (currentText.trim()) {
        contentBlocks.push({ type: 'text', text: currentText });
      }
      if (contentBlocks.length > 0) {
...
function summarizeToolInput(toolName: string, input: unknown): string | undefined {
  ...
  if (toolName === 'Edit' && Array.isArray(data.files)) {
    const files = data.files
      .map((file) => {
        ...
        return filePath ? `${kind}: ${filePath}` : null;
      })
