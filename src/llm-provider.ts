...
import type { LLMProvider, StreamChatParams, FileAttachment } from 'claude-to-im-source/src/lib/bridge/host.js';
 ...
 * (image blocks + text + document blocks). Otherwise returns the plain text string.
 *
 * Supported file types:
 * - Images (png, jpeg, gif, webp) → image content blocks
 * - PDFs → document content blocks (Claude native PDF support)
 * - Text files (txt, md, json, js, ts, etc.) → inlined as text blocks
 */
function buildPrompt(
  ...
  const textFiles = files.filter(f => isTextMime(f.type) && !SUPPORTED_IMAGE_TYPES.has(f.type));
  const otherFiles = files.filter(f =>
    !SUPPORTED_IMAGE_TYPES.has(f.type) &&
    !SUPPORTED_DOC_TYPES.has(f.type) &&
    !isTextMime(f.type),
  );

  if (imageFiles.length === 0 && docFiles.length === 0 && textFiles.length === 0 && otherFiles.length === 0) {
    return text;
  }
  ...
  for (const file of textFiles) {
    try {
      ...
      contentBlocks.push({
        type: 'text',
        text: `[File: ${file.name} — failed to decode as text]`,
      });
    }
  }
  ...
  for (const file of otherFiles) {
    contentBlocks.push({
      type: 'text',
      text: `[File: ${file.name} (${file.type}, ${file.size} bytes) — binary file attached but not displayable]`,
    });
  }
...
/**
 * Try to read a file from disk and package it as a FileAttachment.
...
function collectOutputFile(
  ...
  const absolutePath = fs.existsSync(filePath) && filePath.startsWith('/')
    ? filePath
    : workingDirectory
  ...
  try {
    ...
    if (!stat.isFile() || stat.size > MAX_OUTBOUND_FILE_BYTES || stat.size === 0) return null;
...
/** Tool names that create/edit files — track their outputs. */
const FILE_WRITE_TOOLS = new Set([
  'Write', 'Edit', 'MultiEdit', 'CreateFile',
  'write_to_file', 'create_file', 'insert_content', 'apply_diff',
]);
...
export interface StreamState {
  ...
  /** Collected output files from Write/Edit tool calls. */
  outputFilesByPath: Map<string, FileAttachment>;
  /** Tool inputs keyed by tool_use_id — used to extract file paths from results. */
  ...
  switch (msg.type) {
    case 'stream_event': {
      ...
      if (
        ...
        if (FILE_WRITE_TOOLS.has(event.content_block.name)) {
          state.pendingToolInputs.set(event.content_block.id, {
            name: event.content_block.name,
            input: {},
          });
        }
      ...
      if (msg.message?.content) {
        for (const block of msg.message.content) {
          if (block.type === 'text' && block.text) {
            ...
            if (FILE_WRITE_TOOLS.has(block.name) && block.input) {
              state.pendingToolInputs.set(block.id, {
                name: block.name,
      ...
      if (Array.isArray(content)) {
        for (const block of content) {
          if (typeof block === 'object' && block !== null && 'type' in block && block.type === 'tool_result') {
            ...
            // Collect output files from successful file write tools
            if (!rb.is_error && state.pendingToolInputs.has(rb.tool_use_id)) {
              const toolInfo = state.pendingToolInputs.get(rb.tool_use_id)!;
              const filePath = (toolInfo.input.file_path || toolInfo.input.path || toolInfo.input.filePath) as string | undefined;
              if (filePath) {
                const attachment = collectOutputFile(state.workingDirectory, filePath);
                if (attachment) {
                  state.outputFilesByPath.set(attachment.filePath || attachment.id, attachment);
                }
              }
              state.pendingToolInputs.delete(rb.tool_use_id);
            }
          }
        }
      }
    ...
    case 'result': {
      ...
      if (msg.subtype === 'success') {
        const outputFiles = Array.from(state.outputFilesByPath.values());
        controller.enqueue(
          sseEvent('result', {
            ...
            ...(outputFiles.length > 0 ? { output_files: outputFiles } : {}),
          }),
        );
      } else {
