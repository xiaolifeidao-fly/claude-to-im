...
export function loadConfig(): Config {
  ...
  return {
    ...
    maxFileSize: env.get("CTI_MAX_FILE_SIZE")
      ? Number(env.get("CTI_MAX_FILE_SIZE"))
      : undefined,
    fileOutputEnabled: env.has("CTI_FILE_OUTPUT_ENABLED")
      ? env.get("CTI_FILE_OUTPUT_ENABLED") !== "false"
      : undefined,
  };
}
...
export function saveConfig(config: Config): void {
  ...
  if (config.maxFileSize !== undefined)
    out += formatEnvLine("CTI_MAX_FILE_SIZE", String(config.maxFileSize));
  if (config.fileOutputEnabled !== undefined)
    out += formatEnvLine("CTI_FILE_OUTPUT_ENABLED", String(config.fileOutputEnabled));
...
export function configToSettings(config: Config): Map<string, string> {
  ...
  if (config.codexDebugEvents !== undefined) {
    m.set("cti_codex_debug_events", String(config.codexDebugEvents));
  }
  if (config.maxFileSize !== undefined) {
    m.set("cti_max_file_size", String(config.maxFileSize));
  }
  if (config.fileOutputEnabled !== undefined) {
    m.set("cti_file_output_enabled", String(config.fileOutputEnabled));
  }
