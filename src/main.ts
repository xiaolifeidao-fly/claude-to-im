...
function patchFeishuCardKitCompatibility(): void {
  try {
    const feishuAdapterPath = new URL(
      './node_modules/claude-to-im/dist/lib/bridge/adapters/feishu-adapter.js',
      import.meta.url,
    );
    const fsPath = feishuAdapterPath.pathname;
    if (!fs.existsSync(fsPath)) return;

    const source = fs.readFileSync(fsPath, 'utf-8');
    if (source.includes('CardKit v2 is unavailable in the installed Feishu SDK')) return;

    const patched = source
      .replace(
        "this.cardCreatePromises=new Map;",
        "this.cardCreatePromises=new Map;this.loggedMissingCardKitV2=!1;",
      )
      .replace(
        "if(!this.restClient||this.activeCards.has(e))return Promise.resolve(!1);",
        "if(!this.restClient||this.activeCards.has(e))return Promise.resolve(!1);if(!this.restClient?.cardkit?.v2?.card)return this.loggedMissingCardKitV2||(this.loggedMissingCardKitV2=!0,console.warn('[feishu-adapter] CardKit v2 is unavailable in the installed Feishu SDK; falling back to normal text replies without streaming cards.')),Promise.resolve(!1);",
      )
      .replace(
        "(this.restClient).cardkit.v2.card.streamContent({",
        "(this.restClient)?.cardkit?.v2?.card?.streamContent&&((this.restClient).cardkit.v2.card.streamContent({",
      )
      .replace(
        "}).then(()=>{",
        "}))?.then(()=>{",
      ...
      .replace(
        "if(!o||!this.restClient)return!1;",
        "if(!o||!this.restClient)return!1;const l=this.restClient?.cardkit?.v2?.card?.settings?.streamingMode?.set,a=this.restClient?.cardkit?.v2?.card?.update;if(!l||!a)return!1;",
      )
      .replace(
    ...
    if (patched !== source) {
      fs.writeFileSync(fsPath, patched, 'utf-8');
      console.log('[claude-to-im] Applied Feishu CardKit compatibility patch');
    }
  } catch (err) {
    console.warn('[claude-to-im] Failed to patch Feishu adapter compatibility:', err);
  }
}
...
async function main(): Promise<void> {
  patchFeishuCardKitCompatibility();
  const config = loadConfig();
  if (config.codexDebugEvents) {
    process.env.CTI_CODEX_DEBUG_EVENTS = 'true';
  }
  setupLogger();
