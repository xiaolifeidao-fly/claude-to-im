...
/** Soft limit for a single Feishu card's markdown body. */
export const FEISHU_CARD_TEXT_SOFT_LIMIT = 12000;
...
export function buildToolProgressMarkdown(tools: ToolCallInfo[]): string {
  ...
  const lines = tools.map((tc) => {
    ...
    const sections: string[] = [`${icon} \`${tc.name}\``];
    const inputDetail = tc.inputDetail ? formatToolDetail('Input', tc.inputDetail) : '';
    const resultDetail = tc.resultDetail ? formatToolDetail('Result', tc.resultDetail) : '';
    if (inputDetail) sections.push(inputDetail);
    if (resultDetail) sections.push(resultDetail);
    return sections.join('\n');
  });
  return lines.join('\n');
}

function formatToolDetail(label: string, detail: string): string {
  const trimmed = detail.trim();
  if (!trimmed) return '';
  const normalized = trimmed.length > 1200 ? `${trimmed.slice(0, 1200)}\n...` : trimmed;
  return `**${label}**\n\`\`\`\n${normalized}\n\`\`\``;
}
...
/**
 * Split long Feishu markdown across multiple cards.
 * Prefers newline boundaries to reduce formatting breakage.
 */
export function splitFeishuCardText(text: string, limit = FEISHU_CARD_TEXT_SOFT_LIMIT): string[] {
  if (!text) return [''];
