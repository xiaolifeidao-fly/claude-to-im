...
async function deliverOutputFiles(
  adapter: BaseChannelAdapter,
  address: ChannelAddress,
  files: import('./host.js').FileAttachment[],
  sessionId: string,
  replyToMessageId?: string,
): Promise<SendResult> {
  if (files.length === 0) return { ok: true };
  console.log(`[bridge-manager] Delivering ${files.length} output file(s) via ${adapter.channelType}: ${files.map(f => f.name).join(', ')}`);
  const result = await deliver(adapter, {
    address,
    text: '',
    parseMode: 'plain',
  ...
  if (!result.ok) {
    console.warn(`[bridge-manager] deliverOutputFiles failed: ${result.error}`);
  }
  return result;
}
...
interface BridgeManagerState {
  ...
  /** Monotonic cancellation generation per session to drop queued work after /stop. */
...
function processWithSessionLock(sessionId: string, fn: () => Promise<void>): Promise<void> {
  ...
  const generation = state.sessionCancelGenerations.get(sessionId) || 0;
  const guarded = async () => {
    if ((state.sessionCancelGenerations.get(sessionId) || 0) !== generation) {
      return;
    }
    await fn();
  };
  const current = prev.then(guarded, guarded);
...
function cancelSessionTasks(
  sessionId: string,
  reason: 'stop' | 'preserve-session' = 'stop',
): { abortedActive: boolean; hadQueuedWork: boolean } {
  const state = getState();
  const currentGeneration = state.sessionCancelGenerations.get(sessionId) || 0;
  state.sessionCancelGenerations.set(sessionId, currentGeneration + 1);

  const taskAbort = state.activeTasks.get(sessionId);
  const abortedActive = Boolean(taskAbort);
  if (taskAbort) {
  ...
  return { abortedActive, hadQueuedWork };
}
...
async function handleMessage(
  ...
  const onToolEvent = hasStreamingCards ? (toolId: string, toolName: string, status: 'running' | 'complete' | 'error', detail?: string) => {
    if (toolName) {
      ...
      if (existing) {
        ...
        if (detail) existing.resultDetail = detail;
      }
    }
    try {
  ...
  try {
    ...
    if (result.outputFiles.length > 0) {
      console.log(`[bridge-manager] LLM produced ${result.outputFiles.length} output file(s), sending to ${adapter.channelType}`);
      const fileResult = await deliverOutputFiles(
        adapter,
        msg.address,
        result.outputFiles,
      ...
      if (!fileResult.ok) {
        console.warn(`[bridge-manager] Failed to deliver output files: ${fileResult.error}`);
      }
    } else {
      console.log(`[bridge-manager] LLM turn completed, no output files to deliver`);
    }

    // Persist the actual SDK session ID for future resume.
    // On any error (including user-initiated abort), clear the stale ID so the
    // next message starts a fresh SDK session backed by the clean conversationHistory
    // from the DB — attempting to resume an aborted session often fails silently
    // and causes the model to lose all prior context.
    if (binding.id) {
      try {
        const update = computeSdkSessionUpdate(
          result.sdkSessionId,
          result.hasError,
          result.preserveSdkSession,
        );
        if (update !== null) {
  ...
  switch (command) {
    case '/start':
      response = [
        ...
        '',
        '<b>Commands:</b>',
        '/new_session [path] - Start new session',
        '/new [path] - Alias of /new_session',
        ...
        '/abort - Stop current task and keep provider session',
    ...
    case '/abort': {
      const binding = router.resolve(msg.address);
      const result = cancelSessionTasks(binding.codepilotSessionId, 'preserve-session');
      if (result.abortedActive || result.hadQueuedWork) {
        response = 'Aborting the current task and preserving the session...';
      } else {
        response = 'No task is currently running in this session.';
      }
      break;
 ...
 * - If abort requested session preservation → keep the existing SDK session
 *   unless a fresh sdkSessionId is available
 * - If result has error → clear to empty string
 * - Otherwise → no update needed
 */
export function computeSdkSessionUpdate(
  sdkSessionId: string | null | undefined,
  hasError: boolean,
  preserveSdkSession = false,
): string | null {
  if (sdkSessionId && !hasError) {
    return sdkSessionId;
  }
  if (preserveSdkSession) {
