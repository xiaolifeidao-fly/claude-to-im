...
/** Valid file_type values accepted by the Feishu im.file.create API. */
const FEISHU_FILE_TYPES = new Set(['opus', 'mp4', 'pdf', 'doc', 'xls', 'ppt', 'stream']);

/**
 * Map a file extension to a Feishu-accepted file_type value.
 * Falls back to 'stream' for any unrecognised extension.
 */
function toFeishuFileType(ext: string): string {
  if (FEISHU_FILE_TYPES.has(ext)) return ext;
  const mapping: Record<string, string> = {
    docx: 'doc', xlsx: 'xls', pptx: 'ppt',
    m4a: 'opus', ogg: 'opus', mp3: 'opus',
  };
  return mapping[ext] || 'stream';
}

/** Extension → MIME type for common text/document formats. */
const EXT_TO_MIME: Record<string, string> = {
  ...
  cs: 'text/x-csharp', swift: 'text/x-swift', dart: 'text/x-dart',
...
/** Return a MIME type for a given file extension, or null if unknown. */
function guessMimeFromExt(ext: string): string | null {
  return EXT_TO_MIME[ext.replace(/^\./, '').toLowerCase()] ?? null;
}

/**
 * Heuristic: detect if a buffer contains UTF-8 text.
 * Returns { mimeType, ext } if it looks like text, null if likely binary.
 */
function detectTextContent(buffer: Buffer): { mimeType: string; ext: string } | null {
  ...
  if (printable / sampleLen < 0.85) return null;

  // Attempt JSON detection on full buffer
  const trimmed = sample.toString('utf-8').trimStart();
...
interface FeishuCardInstance {
  cardId: string;
  messageId: string;
  cardApiVersion: 'v1' | 'v2';
  sequence: number;
}

/** State for an active streaming card session. */
interface FeishuCardState {
  cards: FeishuCardInstance[];
  startTime: number;
  toolCalls: ToolCallInfo[];
  thinking: boolean;
  pendingText: string | null;
  lastUpdateAt: number;
  lastFlushTokenEstimate: number;
  throttleTimer: ReturnType<typeof setTimeout> | null;
}

/** Approximate token delta required before pushing another streaming update. */
const CARD_UPDATE_TOKEN_STEP = 100;

/** Safety timer so low-volume streams still update eventually. */
const CARD_UPDATE_MAX_DELAY_MS = 1500;

function estimateTokenCount(text: string): number {
  if (!text) return 0;
  let estimate = 0;
  for (const char of text) {
    estimate += char.charCodeAt(0) <= 0x7f ? 0.25 : 0.75;
  }
...
export class FeishuAdapter extends BaseChannelAdapter {
  ...
  /** Warn once if the installed Feishu SDK lacks CardKit v2 APIs. */
  private loggedMissingCardKitV2 = false;
  ...
  private createStreamingCard(chatId: string, replyToMessageId?: string): Promise<boolean> {
    ...
    if (!this.getCardApiVersion()) {
      if (!this.loggedMissingCardKitV2) {
        this.loggedMissingCardKitV2 = true;
        console.warn(
          '[feishu-adapter] CardKit APIs are unavailable in the installed Feishu SDK; ' +
          'falling back to normal text replies without streaming cards.',
        );
      }
  ...
  private async _doCreateStreamingCard(chatId: string, replyToMessageId?: string): Promise<boolean> {
    ...
    try {
      ...
      const initialCard = await this.createCardInstance(chatId, replyToMessageId);
      if (!initialCard) {
        this.activeCards.delete(chatId);
        return false;
      }
      this.activeCards.get(chatId)?.cards.push(initialCard);
      console.log(`[feishu-adapter] Streaming card created: cardId=${initialCard.cardId}, msgId=${initialCard.messageId}`);
      return true;
    } catch (err) {
  ...
  private async createCardInstance(chatId: string, replyToMessageId?: string): Promise<FeishuCardInstance | null> {
    if (!this.restClient) return null;

    const cardApiVersion = this.getCardApiVersion();
    if (!cardApiVersion) return null;

    if (cardApiVersion === 'v1' && !this.loggedUsingCardKitV1) {
      this.loggedUsingCardKitV1 = true;
      console.warn('[feishu-adapter] CardKit v2 unavailable; using CardKit v1 card updates for streaming preview.');
    }
    ...
    const createResp = await this.getCardApi(cardApiVersion)?.create({
  ...
  /**
   * Update streaming card content with token-based batching.
   */
  private updateCardContent(chatId: string, text: string, options?: { force?: boolean }): void {
    ...
    const tokenDelta = tokenEstimate - state.lastFlushTokenEstimate;
    const elapsed = state.lastUpdateAt > 0 ? Date.now() - state.lastUpdateAt : Number.POSITIVE_INFINITY;

    if (!options?.force && state.lastUpdateAt > 0 && tokenDelta < CARD_UPDATE_TOKEN_STEP) {
      if (elapsed >= CARD_UPDATE_MAX_DELAY_MS) {
      ...
      if (!state.throttleTimer) {
        state.throttleTimer = setTimeout(() => {
          state.throttleTimer = null;
          this.flushCardUpdate(chatId);
        }, CARD_UPDATE_MAX_DELAY_MS - elapsed);
      }
      return;
    }
  ...
  private flushCardUpdate(chatId: string): void {
    ...
    const content = buildStreamingContent(state.pendingText || '', state.toolCalls);
    const chunks = splitFeishuCardText(content);

    this.syncStreamingCards(chatId, chunks).then(() => {
      state.lastUpdateAt = Date.now();
      state.lastFlushTokenEstimate = estimateTokenCount(content);
    }).catch((err: unknown) => {
  ...
  private updateToolProgress(chatId: string, tools: ToolCallInfo[]): void {
    ...
    this.updateCardContent(chatId, state.pendingText || '', { force: true });
  }
  ...
  private async finalizeCard(
    ...
    try {
      ...
      const responseChunks = splitFeishuCardText(responseText);
      await this.ensureStreamingCardCount(chatId, responseChunks.length);

      for (let i = 0; i < responseChunks.length; i++) {
        const card = state.cards[i];
        const chunkFooter = responseChunks.length > 1
          ? {
              status: i === 0 ? `${footer.status} · Part ${i + 1}/${responseChunks.length}` : `Continued · Part ${i + 1}/${responseChunks.length}`,
              elapsed: i === 0 ? footer.elapsed : '',
            }
          : footer;

        if (card.cardApiVersion === 'v2') {
          const streamingModeSet = (this.restClient as any)?.cardkit?.v2?.card?.settings?.streamingMode?.set;
          if (streamingModeSet) {
            card.sequence++;
            await streamingModeSet({
              path: { card_id: card.cardId },
              data: { streaming_mode: false, sequence: card.sequence },
            });
          }
        }

        card.sequence++;
        await this.replaceCardByVersion(
          card,
          buildFinalCardJson(responseChunks[i] || '', i === 0 ? state.toolCalls : [], chunkFooter),
        );
      }

      console.log(`[feishu-adapter] Card finalized: cardId=${state.cards[0]?.cardId}, status=${status}, elapsed=${formatElapsed(elapsedMs)}`);
      return true;
    } catch (err) {
  ...
  private getCardApiVersion(): 'v1' | 'v2' | null {
    if ((this.restClient as any)?.cardkit?.v2?.card) return 'v2';
    if ((this.restClient as any)?.cardkit?.v1?.card) return 'v1';
    return null;
  }

  private getCardApi(version: 'v1' | 'v2'): any {
    return version === 'v2'
      ? (this.restClient as any)?.cardkit?.v2?.card
      : (this.restClient as any)?.cardkit?.v1?.card;
  }

  private async updateCardByVersion(card: FeishuCardInstance, content: string): Promise<void> {
    if (card.cardApiVersion === 'v2') {
      const streamContent = (this.restClient as any)?.cardkit?.v2?.card?.streamContent;
      if (!streamContent) throw new Error('CardKit v2 streamContent is unavailable');
      await streamContent({
        path: { card_id: card.cardId },
        data: { content, sequence: card.sequence },
      });
      return;
    }

    await this.replaceCardByVersion(card, buildFinalCardJson(content, [], null));
  }

  private async replaceCardByVersion(card: FeishuCardInstance, cardJson: string): Promise<void> {
    if (card.cardApiVersion === 'v2') {
      const updateCard = (this.restClient as any)?.cardkit?.v2?.card?.update;
      if (!updateCard) throw new Error('CardKit v2 update is unavailable');
      await updateCard({
        path: { card_id: card.cardId },
        data: { type: 'card_json', data: cardJson, sequence: card.sequence },
      });
      return;
    }

    const updateCard = (this.restClient as any)?.cardkit?.v1?.card?.update;
    if (!updateCard) throw new Error('CardKit v1 update is unavailable');
    await updateCard({
      path: { card_id: card.cardId },
      data: {
  ...
  private async ensureStreamingCardCount(chatId: string, neededCount: number): Promise<boolean> {
    const state = this.activeCards.get(chatId);
    if (!state) return false;

    while (state.cards.length < neededCount) {
      const card = await this.createCardInstance(chatId);
      if (!card) return false;
      state.cards.push(card);
      console.log(`[feishu-adapter] Continuation streaming card created: cardId=${card.cardId}, msgId=${card.messageId}`);
    }

    return true;
  }

  private async syncStreamingCards(chatId: string, chunks: string[]): Promise<void> {
    const state = this.activeCards.get(chatId);
    if (!state) return;

    const ok = await this.ensureStreamingCardCount(chatId, chunks.length);
    if (!ok) return;

    for (let i = 0; i < chunks.length; i++) {
      const card = state.cards[i];
      card.sequence++;
      await this.updateCardByVersion(card, chunks[i]);
    }
  }
  ...
  private async sendAttachments(
    ...
    console.log(`[feishu-adapter] sendAttachments: ${attachments.length} file(s) to chat=${chatId}`);
    let lastMessageId: string | undefined;

    if (leadingText && leadingText.trim()) {
      const leadResult = await this.sendAsPost(chatId, leadingText);
      if (!leadResult.ok) return leadResult;
      lastMessageId = leadResult.messageId;
    }

    for (let i = 0; i < attachments.length; i++) {
      const attachment = attachments[i];
      const uploadResult = await this.uploadFileAttachment(attachment);
      if (!uploadResult.ok || (!uploadResult.fileKey && !uploadResult.imageKey)) {
        console.warn(`[feishu-adapter] sendAttachments: upload failed for ${attachment.name}: ${uploadResult.error}`);
        return { ok: false, error: uploadResult.error || 'File upload failed' };
      }

      try {
        const payload = uploadResult.imageKey
          ? {
        ...
        const res = (replyToMessageId && i === 0)
        ...
        if (!res?.data?.message_id) {
          return { ok: false, error: res?.msg || 'File send failed' };
        }
  ...
  ): Promise<{ ok: boolean; fileKey?: string; imageKey?: string; error?: string }> {
    if (!this.restClient) {
      return { ok: false, error: 'Feishu client not initialized' };
    }

    // Images use a separate upload endpoint and return image_key
    if (attachment.type?.startsWith('image/')) {
      console.log(`[feishu-adapter] Uploading image: ${attachment.name} (${attachment.size} bytes)`);
      try {
        const res = await (this.restClient as any).im.image.create({
          data: {
            image_type: 'message',
        ...
        if (imageKey) {
          console.log(`[feishu-adapter] Image uploaded: ${attachment.name} → ${imageKey}`);
          return { ok: true, imageKey };
        }
        console.warn(`[feishu-adapter] Image upload failed (no image_key): code=${res?.code} msg=${res?.msg} res=${JSON.stringify(res)}`);
        return { ok: false, error: res?.msg || 'Feishu image upload returned no image_key' };
      } catch (err) {
        console.warn(`[feishu-adapter] Image upload error:`, err instanceof Error ? err.message : err);
        return { ok: false, error: err instanceof Error ? err.message : 'Feishu image upload failed' };
      }
    }

    // Non-image files use im.file.create with a mapped file_type
    const safeName = path.basename(attachment.name || 'artifact.bin').replace(/[^\w.\- ]+/g, '_');
    const ext = path.extname(safeName).replace(/^\./, '').toLowerCase() || 'bin';
    const fileType = toFeishuFileType(ext);
    console.log(`[feishu-adapter] Uploading file: ${attachment.name} → ${safeName} (${attachment.size} bytes, file_type=${fileType})`);

    try {
      const res = await (this.restClient as any).im.file.create({
        data: {
          file_type: fileType,
          file_name: safeName,
      ...
      if (fileKey) {
        console.log(`[feishu-adapter] File uploaded: ${safeName} → ${fileKey}`);
        return { ok: true, fileKey };
      }
      console.warn(`[feishu-adapter] File upload failed (no file_key): code=${res?.code} msg=${res?.msg} res=${JSON.stringify(res)}`);
      return { ok: false, error: res?.msg || 'Feishu file upload returned no file_key' };
    } catch (err) {
      console.warn(`[feishu-adapter] File upload error:`, err instanceof Error ? err.message : err);
      return { ok: false, error: err instanceof Error ? err.message : 'Feishu file upload failed' };
    }
  }
  ...
  private async sendAsCard(chatId: string, text: string): Promise<SendResult> {
    ...
    return this.sendCardJson(chatId, cardContent, text);
  }

  private async sendCardJson(chatId: string, cardContent: string, fallbackText?: string): Promise<SendResult> {
    if (!this.restClient) {
      return { ok: false, error: 'Feishu client not initialized' };
    }
  ...
  private async processIncomingEvent(data: FeishuMessageEventData): Promise<void> {
    ...
    if (messageType === 'text') {
      ...
      const { fileKey, fileName } = this.extractFileInfo(msg.content);
      if (fileKey) {
        const resourceType = messageType === 'audio' || messageType === 'video' || messageType === 'media'
          ? messageType
          : 'file';
        const attachment = await this.downloadResource(msg.message_id, fileKey, resourceType, fileName || undefined);
        if (attachment) {
          attachments.push(attachment);
        } else {
  ...
  private extractFileKey(content: string): string | null {
    return this.extractFileInfo(content).fileKey;
  }

  /**
   * Extract both file key and original file name from message content JSON.
   * Feishu file messages may include file_name alongside file_key.
   */
  private extractFileInfo(content: string): { fileKey: string | null; fileName: string | null } {
    try {
      const parsed = JSON.parse(content);
      const fileKey = parsed.image_key || parsed.file_key || parsed.imageKey || parsed.fileKey || null;
      const fileName = parsed.file_name || parsed.fileName || null;
   ...
   *
   * @param originalName - Original filename from the message content, used to
   *   determine the correct MIME type and preserve the human-readable name.
   */
  private async downloadResource(
    ...
    try {
      ...
      // Resolve name and MIME type — prefer original filename when available
      let name: string;
      let mimeType: string;

      if (resourceType === 'image') {
        mimeType = MIME_BY_TYPE['image'] || 'image/png';
        name = originalName || `${fileKey}.png`;
      } else if (resourceType === 'audio') {
        mimeType = MIME_BY_TYPE['audio'] || 'audio/ogg';
        name = originalName || `${fileKey}.ogg`;
      } else if (resourceType === 'video' || resourceType === 'media') {
        mimeType = MIME_BY_TYPE['video'] || 'video/mp4';
        name = originalName || `${fileKey}.mp4`;
      } else if (originalName) {
        // Non-media file with a known original name: infer MIME from extension
        name = originalName;
        const guessed = guessMimeFromExt(path.extname(originalName));
        mimeType = guessed || 'application/octet-stream';
      } else {
        // No original name: try content-based detection, then fall back to .bin
        const detected = detectTextContent(buffer);
        if (detected) {
          mimeType = detected.mimeType;
          name = `${fileKey}.${detected.ext}`;
        } else {
      ...
      console.log(`[feishu-adapter] Resource downloaded: ${buffer.length} bytes, key=${fileKey}, name=${name}, type=${mimeType}`);
