 ...
 * - bridge-manager: sdkSessionId update logic
 */
...
describe('bridge-manager - computeSdkSessionUpdate', () => {
  ...
  it('preserves existing session on abort when no fresh sdkSessionId is available', async () => {
    const { computeSdkSessionUpdate } = await import('../../lib/bridge/bridge-manager');
    const result = computeSdkSessionUpdate(null, true, true);
    assert.equal(result, null, 'Preserve-session abort should keep existing binding value');
  });

  it('persists a fresh sdkSessionId on preserve-session abort when available', async () => {
    const { computeSdkSessionUpdate } = await import('../../lib/bridge/bridge-manager');
    const result = computeSdkSessionUpdate('new-sdk-123', true, true);
    assert.equal(result, 'new-sdk-123', 'Preserve-session abort should keep the latest SDK ID');
  });
});
